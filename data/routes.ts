...
export const routes: Route[] = [
  ...
  },
  {
    ...
    verification: {
      ...
      verifiedDate: '2026-09-07',
      reviewDueDate: '2026-09-12',
      note: 'Route Verification Refresh, 12 September due-soon batch (7 September 2026): a genuine evidence upgrade — the page\'s own "Book with Jet2 Holidays" and "Book with Tui" links name two current operators not previously recorded here (the prior check found the page live but did not extract operator names). Both are in the airline catalogue (data/airlines.ts) and are added to airlineSlugs. No duration, frequency or seasonality statement exists on this page to establish genuine route-specific continuity, matching exactly the evidence-depth finding already made for the sibling leeds-bradford-antalya record (Rolling Reverification Batch 6, 4 September 2026) — that record\'s own reasoning applies identically here: verifiedDate is updated to reflect the genuine operator-name upgrade, but reviewDueDate is deliberately NOT recalculated to a fresh cadence-policy window from this session\'s new verifiedDate, since doing so would administratively extend a review this evidence does not yet support classifying as either STABLE or RECENT/CHANGING. Left on the existing 2026-09-12 date pending either a genuine future evidence upgrade (duration/frequency/seasonality) or a founder decision on how this class of thin-but-verified record should be handled generally.',
    },
    airlineVerifications: ['jet2', 'tui'].map((airlineSlug) => ({
      airlineSlug,
      status: 'verified' as const,
  ...
  },
  {
    ...
    verification: {
      ...
      verifiedDate: '2026-08-12',
      reviewDueDate: '2026-09-12',
      note: 'Gatwick publishes direct Marrakech service in its Africa route table, and its current flight information shows Marrakech departures. Exact operators, dates, frequency and duration vary; Trip.com remains unavailable for an airport-specific dateless handoff. Route Verification Refresh, 12 September due-soon batch (7 September 2026): a fresh research attempt was made -- Gatwick\'s own "Browse all destinations" link now resolves to a third-party booking aggregator rather than an official destination page, and direct URL attempts at easyJet, TUI and British Airways Marrakech-specific pages did not resolve. The only Marrakech-specific evidence reopened is the same static press release already dated 19 May 2025 (identified in this record\'s sourceUrl) -- reopening it today does not establish current 2026 operator or frequency facts, exactly the reasoning already applied to the sibling london-gatwick-agadir record (Rolling Reverification Batch 6). verifiedDate and reviewDueDate are therefore deliberately left unchanged rather than administratively extended on this stale source.',
    },
    intro:
