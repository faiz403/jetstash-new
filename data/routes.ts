...
export const routes: Route[] = [
  ...
  },
  {
    ...
    verification: {
      ...
      verifiedDate: '2026-09-04',
      reviewDueDate: '2026-10-04',
      note: 'Route Intelligence Freshness rolling-review Batch 3 (4 September 2026): reconfirmed via the same primary source, still live, matching exactly -- 3h30m, Ryanair named as the current direct operator. easyJet\'s winter-2026 service (from a separate April 2026 announcement, not independently re-observed on the destinations page this session) is retained as previously recorded, not newly reconfirmed. Corrected on PR review (cadence validation) from an initial STABLE call to RECENT / CHANGING (30-day window): matching duration and a confirmed operator prove the route is currently correct, but the same destination page\'s own "Seasons: High = June-August; Low = November-February" framing, combined with Agadir\'s genuinely mixed UK winter-sun/summer travel pattern, means the evidence does not affirmatively rule out seasonal instability -- the STABLE bar requires that, not just current accuracy.',
    },
    intro:
  ...
  },
  {
    ...
    verification: { status: 'verified', sourceName: "Birmingham Airport's official Faro destination page", sourceUrl: 'https://airport.birminghamairport.co.uk/destinations/where-we-fly/faro/', verifiedDate: '2026-09-04', reviewDueDate: '2026-10-04', note: 'Route Intelligence Freshness rolling-review Batch 3 (4 September 2026): reconfirmed via the same primary source, still live -- names easyJet, Jet2.com and TUI (matching the existing record) plus Ryanair, not previously recorded here. Ryanair is not currently in the airline catalogue (data/airlines.ts), so it is noted here rather than added to the public airlineSlugs/frequency copy -- a separate decision, out of scope for a routine metadata refresh. The page states an approximate 1h50m flight time, but the existing "timings vary by source" framing is left unchanged rather than overwritten from a single reading, since that framing was deliberately chosen to reflect a genuine known discrepancy across sources. Corrected on PR review (cadence validation) from an initial STABLE call to RECENT / CHANGING (30-day window): four confirmed current operators prove route maturity, but the same page\'s own "Seasons: High = July-August; Low = November-February" framing means the evidence does not affirmatively rule out seasonal instability -- the STABLE bar requires that, not just an accurate operator count.' },
  ...
  },
  {
    ...
    verification: { status: 'verified', sourceName: "Birmingham Airport's official Rome destination page", sourceUrl: 'https://airport.birminghamairport.co.uk/destinations/where-we-fly/rome/', verifiedDate: '2026-09-04', reviewDueDate: '2026-10-04', note: 'Route Intelligence Freshness rolling-review Batch 3 (4 September 2026): reconfirmed via the same primary source, still live, matching exactly -- 2h35m, Jet2.com confirmed direct (matching the existing record), plus WIZZ and Ryanair named, not previously recorded here. Neither is currently in the airline catalogue (data/airlines.ts), so both are noted here rather than added to public copy -- a separate decision, out of scope for a routine metadata refresh. Corrected on PR review (cadence validation) from an initial STABLE call to RECENT / CHANGING (30-day window): the exact duration match and three confirmed operators prove the route is currently correct, but no primary source affirmatively rules out seasonal instability -- the STABLE bar requires that, not just current accuracy.' },
