...
export const routes: Route[] = [
  ...
  },
  {
    ...
    destinationSlug: 'agadir',
    flightTime: 'Published duration not recorded in this evidence check; check the exact flight for your dates',
    frequency: 'Gatwick\'s own route table names easyJet, British Airways and TUI, at 8 times weekly; exact dates and duration vary by flight',
    airlineSlugs: ['easyjet', 'british-airways', 'tui'],
    isDirect: true,
    verification: {
      status: 'verified',
      ...
      verifiedDate: '2026-09-04',
      reviewDueDate: '2026-09-12',
      note: 'Rolling Reverification Batch 6 (4 September 2026): a genuine evidence upgrade -- this session\'s check read the source\'s own route table in full and found a specific, dated operator and frequency entry not previously extracted: "Agadir, Morocco -- EasyJet, British Airways, TUI -- 8 x weekly". All three are in the airline catalogue (data/airlines.ts) and are added to airlineSlugs -- these facts are real and genuinely sourced. Corrected twice on PR review: a first correction replaced a Tangier/Dubai-precedent citation with the fact that this press release is independently dated 19 May 2025, over 16 months before this check, and argued that finding the same page still live in September 2026 constituted 16 months of confirmed continuity. On final pre-merge review that reasoning does not hold: this is a single static press release, re-read -- not two temporally distinct observations. A static document that has not been edited since May 2025 says nothing about whether the underlying service still operates on the same terms in September 2026; no genuinely current, live Gatwick source (a live route/schedule page, not an archived press release) was checked to corroborate the 2025 facts against today. STABLE is withdrawn. verifiedDate is kept at 2026-09-04 (the operator/frequency facts are real and worth recording), but reviewDueDate is restored to its original 2026-09-12 rather than extended on unproven continuity -- this route\'s maturity/cadence remains genuinely unresolved pending either a live, current corroborating source or a founder decision on how this class of record should be handled. Duration remains unpublished; Trip.com remains unavailable for an airport-specific dateless handoff.',
    },
    airlineVerifications: ['easyjet', 'british-airways', 'tui'].map((airlineSlug) => ({
      ...
      reviewDueDate: '2026-09-12',
      effectivePeriod: 'Current at access',
      supportedClaim: 'Named as flying direct from Gatwick to Agadir, 8 times weekly, on Gatwick\'s own Africa route table.',
      remainingUncertainty: 'Exact duration and each operator\'s individual share of the 8-weekly total are not stated on this table.',
    })),
    intro:
      'London Gatwick\'s own route table names easyJet, British Airways and TUI flying direct to Agadir, 8 times weekly. Exact duration and the airline for your specific date still vary, so check the exact flight before booking. Trip.com does not currently provide an airport-specific dateless handoff for Gatwick.',
    bookingWindowNote:
      'This guide is route context, not a booking prediction. Check the exact service, duration, fare conditions and baggage terms before booking.',
    peakPeriodIds: [],
  },
