...
6. **Sign up for TravelUp's real affiliate programme and add the tracking parameters.** Every outbound booking link ("Check live price[s]", "Search live prices") is generated centrally by `lib/booking-providers.ts` and currently points at TravelUp (Skyscanner's affiliate application was declined pre-launch, so it's disabled there — see that file's header comment). `BOOKING_PROVIDERS.travelup.affiliateParams` is still an empty placeholder. TravelUp's affiliate programme runs through Commission Junction (signup: `https://signup.cj.com/member/signup/publisher/?cid=6248437`) or their own page (`https://www.travelup.com/en-us/company/affiliate-programme` — check for a UK-locale equivalent). Sign up to get the real tracking parameters. To re-enable Skyscanner in future, flip its `enabled` flag and add its params in the same file — nothing else in the app needs to change.
7. **Get TravelUp's real deep-link URL schema and turn deep-linking back on.** A first attempt guessed a `/flights/search?origin=...` URL shape for route-specific links; in production this landed users on a TravelUp error page and lost their search. `BOOKING_PROVIDERS.travelup.supportsDeepLink` is now deliberately `false`, so every booking link safely falls back to TravelUp's real homepage instead of a broken deep link. Once the affiliate signup above gets you TravelUp's real integration docs, update `getRouteBookingUrl` in `lib/booking-providers.ts` with their actual URL structure and flip `supportsDeepLink` to `true`.
