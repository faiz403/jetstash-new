...
6. **Add TravelUp's real affiliate tracking parameters.** Every outbound booking link ("Check live price[s]", "Search live prices") is generated centrally by `lib/booking-providers.ts` and currently points at TravelUp (Skyscanner's affiliate application was declined pre-launch, so it's disabled there — see that file's header comment). `BOOKING_PROVIDERS.travelup.affiliateParams` is still an empty placeholder; add the real tracking parameters once TravelUp issues them. To re-enable Skyscanner in future, flip its `enabled` flag and add its params in the same file — nothing else in the app needs to change.
7. **Verify TravelUp's real deep-link query schema.** `lib/booking-providers.ts` builds outbound URLs with `origin`/`destination`/`cabinClass` query parameters as a best-effort convention — this has not been confirmed against TravelUp's actual integration docs. Verify (or correct) the parameter names before launch so links land on a real results page rather than a generic search.
