...
const batch = fareObservations.filter((o) => o.id.includes('20260911-8w-v1'));

describe('11 September fixed-date Fare Watcher evidence', () => {
  it('contains only the four authorised routes in collection order', () => {
    expect(batch.map((o) => [o.routeSlug, o.price])).toEqual([
      ...
      ['manchester-lahore', 412],
      ['manchester-islamabad', 415],
      ['birmingham-mumbai', 446],
    ]);
  });

  it.each(batch)('$routeSlug is complete, profile-consistent, and remains poor-itinerary evidence', (o) => {
    const prior = fareObservations.find((p) => p.routeSlug === o.routeSlug && p.id.includes('20260910-8w-v1'));
    expect(prior).toBeDefined();
    expect(o.profileId).toBe(prior!.profileId);
    expect([o.departureDate, o.returnDate, o.observedDate]).toEqual(['2026-11-03', '2026-11-17', '2026-09-11']);
    ...
    expect(isObservationPublishable(o, getRouteBySlug(o.routeSlug), '2026-09-11')).toBe(true);
  });
});
