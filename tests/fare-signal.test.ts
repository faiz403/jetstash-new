...
describe('Fare Signal production coverage counts', () => {
  it('reports 22 routes with a current publishable fare and 13 without one at the current archive date', () => {
    ...
    expect(routes.filter((route) => getTripComRouteUrl(route.slug)).length).toBe(26);
  });
});
