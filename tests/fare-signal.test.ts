...
describe('Fare Signal production coverage counts', () => {
  it('reports 22 routes with a current publishable fare and 10 without one at the current archive date', () => {
    ...
    expect(routes.filter((route) => getTripComRouteUrl(route.slug)).length).toBe(23);
  });
});
