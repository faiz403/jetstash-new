...
describe('computeBookBySnapshot — unsupported inputs return null rather than guessing', () => {
  ...
  describe('verification-pending leakage fix (audit finding): a priority route whose verification has since expired — fully deterministic on the `now` parameter, no system-clock mocking needed', () => {
    ...
    it('sanity: the fixture route\'s real reviewDueDate is 2026-10-14 (refreshed by the 14 September 2026 Route Verification Maintenance batch, was previously 2026-09-14)', () => {
      expect(route.verification?.reviewDueDate).toBe('2026-10-14');
    });
    ...
    it('2026-10-15 (the day after reviewDueDate): stops producing a snapshot, even though the route is still in BOOK_BY_PRIORITY_ROUTE_SLUGS and still has upcoming peak-period occurrences', () => {
      expect(computeBookBySnapshot('manchester-lahore', new Date('2026-10-15T12:00:00Z'))).toBeNull();
    });
