...
describe('Manchester-Doha\'s original fare observation is untouched by this round', () => {
  it('same id, price, dates as before — no fabricated observation was added to close its gap', () => {
    ...
    // Three further genuinely separate observations were added later - by
    ...
    // (obs-man-doh-economy-20260818-8w-v1), and by the Tuesday full weekly
    // refresh on 1 September 2026 (obs-man-doh-economy-20260901-8w-v1) -
    // this test's own scope is only that THIS specific historic record
    // (20260805) was never edited, not that no further observation could
    // ever legitimately be added.
    expect(getPublishableObservationsByRoute('manchester-doha', NOW_ISO).length).toBe(4);
  });
