...
describe('Manchester-Dubai\'s new fare observation matches the approved specification exactly', () => {
  ...
  it('Manchester-Dubai\'s two historic entries remain untouched — one further genuine observation was added later (18 August 2026, Weekly Full Fare Refresh #1)', () => {
    const dubaiObs = fareObservations.filter((o) => o.routeSlug === 'manchester-dubai');
    expect(dubaiObs.length).toBe(4);
    const historic = dubaiObs.filter((o) => o.id !== NEW_DUBAI_OBS_ID && o.id !== 'obs-man-dxb-economy-20260818-8w-v1');
...
describe('The audit and fare-archive documents accurately reflect the closed observation and its content-depth review', () => {
  ...
  it('states the real, current fare-tracking route count (78 of 88, updated 18 August 2026 — london-gatwick-ahmedabad dropped out when Route Verification Refresh Batch 1\'s correction reclassified it unverified), not a stale hand-typed figure', () => {
    const totalTracked = routes.filter((r) => getPublishableObservationsByRoute(r.slug, NOW_ISO).length > 0).length;
    expect(totalTracked).toBe(78);
