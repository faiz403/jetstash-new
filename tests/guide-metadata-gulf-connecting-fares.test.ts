...
// Updated 10 Sept 2026 (Astra bounded review, unrelated to this file's own
// metadata fix above): paragraph 2's "your bags are checked through" was
// corrected — a single-ticket itinerary guarantees connection protection,
// not that baggage is necessarily checked all the way through, since that
// depends on the actual ticket/airline arrangement. This constant reflects
// the current approved body text; the metadata-only-fix assertion below
// still holds, since paragraphs are unchanged by *this file's own* title/
// summary fix.
...
describe('Astra bounded trust review (10 Sept 2026) — finding #2, baggage-through-booking wording', () => {
  ...
  it('still states the itinerary-protection benefit of a single ticket, and now says the baggage outcome depends on the actual arrangement', () => {
    const body = guide().paragraphs.join(' ');
    expect(body).toMatch(/the airline owns the problem if you misconnect/i);
