...
**Implemented locally, not yet deployed or production-verified:**
- **TR-017** (P1): Birmingham–Mumbai connecting-route coverage gap — a `birmingham-mumbai` Route record now exists locally on branch `feature/birmingham-mumbai-route`, rebased on the verification-pending leakage fix, modelled as connecting only (`isDirect: false`), sourced to Birmingham Airport's own route-specific Mumbai destination page (re-verified live this pass; an earlier draft's incorrect Doha/airline-hub-mapping claim was found and corrected — see TR-017's own section). Typecheck, lint, the full test suite (200/200, including 28 route-specific tests), and a local production build all pass. **Not pushed, not merged into `main`, not deployed, and not production-verified** — treat as open until it reaches `jetstash.co.uk` and is independently confirmed live. See TR-017's own section below for full evidence and what was intentionally left unverified (flight duration, frequency, airline-hub mapping, demand periods, fare).
...
- **Resolution required:** Add a `birmingham-mumbai` route, modelled as connecting (never direct), with its own airline/frequency evidence. **Implemented on local branch `feature/birmingham-mumbai-route`**, rebased on top of the verification-pending leakage fix (commit `8b080f6b4d1253acff3b02b9cdadd8e84dbed854`): a `Route` record was added to `data/routes.ts` and `birmingham` was added to `mumbai.ukAirports` in `data/destinations.ts`.
  - **Correction (this pass):** an earlier draft of this route incorrectly stated "Qatar Airways (via Doha)" in its `intro` and listed "Doha" in `frequency`. **Birmingham Airport's own page never mentions Doha, and never maps any airline to any specific hub** — that draft inferred Qatar Airways' general hub from outside knowledge, not from the cited source. Corrected below; the live source was re-fetched and re-read in full before making this correction, not worked from the earlier summary.
  - **Evidence, re-verified live** (`https://airport.birminghamairport.co.uk/destinations/where-we-fly/mumbai/`, accessed 2026-07-23):
    - Supports: *"There are currently no direct flights to Mumbai, so you'll need to connect through a major hub including, but not limited to, Amsterdam, Delhi, Dubai, Istanbul or Paris before continuing on to Mumbai."* — the no-direct-service claim, and that Amsterdam/Delhi/Dubai/Istanbul/Paris are hub cities the airport associates with reaching Mumbai (list explicitly non-exhaustive).
    - Supports: *"Airlines that fly from BHX to Mumbai include: Air France, Air India, Emirates, KLM, Lufthansa, Qatar Airways, Turkish Airlines, Saudia Airlines and Swiss Air."* — these 9 airlines are named as flying the route. Of these, only Air India, Emirates, Qatar Airways, Saudia and Turkish Airlines exist in `data/airlines.ts`; Air France, KLM, Lufthansa and Swiss are omitted from `airlineSlugs` purely because they have no registry entry, not because the source doesn't name them.
    - **Does not support and is not claimed anywhere in the route's copy:** which airline uses which hub (the two lists are presented separately, never paired); Doha as a hub for this route specifically (Doha is not mentioned on the page at all — a general Qatar Airways hub fact from outside the source must not be imported here); total journey duration; frequency/schedule; route-specific demand periods, booking timing, or fare urgency; self-transfer risk for any named itinerary. `flightTime` states duration "varies... and has not been independently established"; `frequency` states connecting options exist and schedules "must be checked"; `peakPeriodIds` is `[]` (no route-specific demand source exists — not inferred from Mumbai's general festival calendar); `bookingWindowNote` gives durable itinerary-comparison advice (journey time, layover, baggage-transfer conditions, ticket flexibility) with no cheapest-period/surge/urgency claim.
    - A route-level `verification` record was added (`status: 'verified'`, source the URL above, verified 2026-07-23, review due 2026-08-23) — scoped explicitly to the no-direct-service claim only; its own `note` field disclaims duration/frequency/demand/fare/self-transfer/hub-mapping by name, so it cannot be misread as verifying the rest. Per the current architecture (see the verification-pending leakage fix, issue 7 correction), this record has no effect on the public "Connecting" display, which already follows from `isDirect: false` alone.
    - A second Birmingham Airport source (a 2019 press release naming Air India's Delhi hub as connecting onward to Mumbai) was found but **not used for any current claim** — 7 years stale, only corroborates the general Delhi-hub network shape, not anything current or route-specific.
  - **Tests:** `tests/birmingham-mumbai-route.test.ts` (28 tests) — route existence/slug relationships, connecting classification via both `getDisplayDirectness` and `getRoutePresentation`, that the verification record supports only the no-direct claim (and explicitly disclaims the rest), that no field of the presentation object claims whole-bundle verification, destination/airport cross-linking (and that existing Manchester/Heathrow Mumbai routes are unaffected), airline-list correctness against the registry intersection, absence of any airline-to-hub mapping, absence of any Doha mention, no invented duration/frequency/urgency/demand claim, empty `peakPeriodIds`, no deal or fare observation, and a sanity check that this route would still be fully suppressed by the shared pending-route gate if it were hypothetically unverified.
  - **Local quality gate:** typecheck, lint, full test suite (200/200 passing), and a local production build all pass. Smoke-tested locally: `/routes/birmingham-mumbai`, metadata, Open Graph image, mobile/desktop overflow, and regressions for the three pending routes plus existing verified/connecting routes — see the session's full verification report for exact results.
- **Status:** **Implemented locally, not deployed, not production-verified.** Not pushed, not merged into `main`, no PR opened for it. Do not read this as closed — it remains open until it reaches `jetstash.co.uk` and is independently confirmed live, per this register's own rule that a blocker "is only fully closed once its correction is deployed to production and confirmed working on jetstash.co.uk."
