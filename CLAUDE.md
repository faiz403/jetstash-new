...
- `data/routes.ts` — specific airport→destination route guides (`Route`), plus the Route Status V1 derivation (`getRouteStatus`, `getEffectiveRoutePresentation`) that models *time-bound* direct-service changes from the append-only ledger in `data/route-status-events.ts` — never as a field on `Route` itself — and `connectingAlternative`, a separate, editorial "realistic 1-stop fallback" field (see README's "Time-bound direct services" section for the full model and the update procedure for a new withdrawal/service-ended event).
